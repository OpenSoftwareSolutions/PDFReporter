package org.oss.pdfreporter.uses.org.apache.commons.beanutils;
...
public class MethodUtils {
     ...
     */
    private static final Map<MethodDescriptor, Method> cache = new HashMap<MethodDescriptor, Method>();
    ...
    public static Method getMatchingAccessibleMethod(
        ...
        for (int i = 0, size = methods.length; i < size ; i++) {
        	// the methods[i].getName returns the long method name in Objective-C, 
        	// we have to do a special compare here to return correct values
            if (methodMatchesOrStartsSame(methods[i].getName(), methodName)) {
    ...
    /**
     * To work in Objective-C we have to return true in case for example "addElement" and the 
     * real Objective-C name "addElementWithJRElement".
     * @param iOSMethodName
     * @param javaMethodName
     * @return true if the iOS method name begins with the java method name and the next string is 
     * "With" or the are the same length (same string).
     */
    private static boolean methodMatchesOrStartsSame(String iOSMethodName, String javaMethodName) {
    	if (iOSMethodName == null || javaMethodName == null) {
    		//this should not happen
    		return false;
    	}
    	if (iOSMethodName.length() >= javaMethodName.length() && iOSMethodName.startsWith(javaMethodName)) {
    		if (iOSMethodName.substring(javaMethodName.length(), iOSMethodName.length()).startsWith("With")
    			|| iOSMethodName.length() == javaMethodName.length()) {
    				return true;
    			}
    	}
