...
public class SetNestedPropertiesRule extends AbstractRule {
//    private Log log = null;
    ...
    public void begin(String namespace, String name, IAttributes attributes) 
                      throws Exception {
        IRules oldRules = digester.getRules();
    ...
    private class AnyChildRules implements IRules {
        private String matchPrefix = null;
        private IRules decoratedRules = null;
        
        private ArrayList<IRule> rules = new ArrayList<IRule>(1);
        ...
        public IDigester getDigester() { return null; }
        public void setDigester(IDigester digester) {}
        ...
        public List<IRule> match(String matchPath) { 
            return match(null,matchPath); 
        }
        
        public List<IRule> match(String namespaceURI, String matchPath) {
            List<IRule> match = decoratedRules.match(namespaceURI, matchPath);
            ...
            if ((matchPath.startsWith(matchPrefix)) &&
                ...
                if ((match == null || match.size()==0)) {
                    ...
                    // so clone it first.
                    LinkedList<IRule> newMatch = new LinkedList<IRule>(match);
        ...
        public void init(String prefix, IRules rules) {
            matchPrefix = prefix;
            decoratedRules = rules;
        }
        
        public IRules getOldRules() {
            return decoratedRules;
        }
    }
    
    private class AnyChildRule extends AbstractRule {
        private String currChildElementName = null;
        
        @Override
        public void begin(String namespaceURI, String name, 
                              IAttributes attributes) throws Exception {
        ...
        public void body(String value) throws Exception {
            ...
            try
            {
            	IRegistry.getIBeansFactory().newBeansUtils().setProperty(top, propName, value);
            }
