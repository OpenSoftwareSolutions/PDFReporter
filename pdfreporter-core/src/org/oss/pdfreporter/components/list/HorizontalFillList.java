...
public class HorizontalFillList extends BaseFillList
{
	...
	private static final Logger log = Logger.getLogger(HorizontalFillList.class.getName());
	...
	public FillPrepareResult prepare(int availableHeight)
	{
		...
		try
		{
			...
			if (filling)
			{
				...
				{
					log.fine("Refilling list row after overflow");
				}
			...
			while(!overflow)
			{
				...
				if (contentsAvailableHeight < contentsHeight)
				{
					...
					if (log.isLoggable(Level.FINE))
					{
						log.fine("Not enough space left for a list row, overflowing");
					}
				...
				else
				{
					...
					if (refillOverflowed)
					{
						...
						if (log.isLoggable(Level.FINE))
						{
							log.fine("Refilling overflowed cell at column "
									+ columnIndex);
						}
					...
					else
					{
						...
						if (log.isLoggable(Level.FINE))
						{
							log.fine("Filling new cell at column "
									+ columnIndex);
						}
			...
			if (overflow)
			{
				if (log.isLoggable(Level.FINE))
				{
					log.fine("List has overflowed at column " + (columnIndex - 1));
				}
	...
	protected void fillRow(int columnCount) throws JRException
	{
		...
		if (log.isLoggable(Level.FINE))
		{
			log.fine("Completed a row with " + columnCount + " columns");
		}
	...
	protected FillListContents getContents(int columnIndex)
	{
		if (columnIndex > 0 && columnIndex >= contentsList.size())
		{
			...
			for (int idx = contentsList.size(); idx <= columnIndex; idx++)
			{
				if (log.isLoggable(Level.FINE))
				{
					log.fine("Cloning list fill contents for column " + idx);
				}
