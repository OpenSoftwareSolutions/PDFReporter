...
public abstract class AbstractTextRenderer
{
	...
	private final static Logger logger = Logger.getLogger(AbstractTextRenderer.class.getName());
...
//		formatWidth = formatWidth < 0 ? 0 : formatWidth;
	...
	{
		IAttributedCharacterIterator paragraph = null;
		...
		while (@AutoreleasePool lineMeasurer.getPosition() < paragraph.getEndIndex() && !isMaxHeightReached)
		{
			...
			while (@AutoreleasePool !lineComplete)
			{
				...
				logger.finest("width: " + availableWidth + ", endLimit: " + tabIndexOrEndIndex + ", requireWord: " + requireNextWord);
				...
				logText(beginIndex,endIndex, layout, paragraph);
...
//						}
						...
						if (layout == null)
						{
							...
							if (nextTabStop.getPosition() == ParagraphUtil.getFirstTabStop(text.getParagraph(), endX).getPosition())//FIXMETAB check based on segments.size()
							{
								...
								//provide dummy maxFontSize because it is used for the line height of this empty line when attempting drawing below
					 			...
					 			ITextLayout tlyt = lbm.nextLayout(100);
			...
			if (drawPosY + lineHeight <= text.getTextHeight())
			{
				...
				for (segmentIndex = 0; segmentIndex < segments.size(); segmentIndex++)
				{
					...
					ITextLayout layout = segment.layout;
...
//				}
	...
	private void logText(int beginIndex, int endIndex, ITextLayout textLayout, IAttributedCharacterIterator paragraph) {
		if (textLayout!=null) {
