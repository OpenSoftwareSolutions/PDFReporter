...
import org.oss.pdfreporter.engine.util.xml.JRXPathExecuter;
import org.oss.pdfreporter.engine.util.xml.JRXPathExecuterUtils;
...
public class JRXmlDataSource extends JRAbstractTextDataSource implements JRRewindableDataSource {
	private final static Logger logger = Logger.getLogger(JRXmlDataSource.class .getName()); 
	...
	{
		this(jasperReportsContext, JRXmlUtils.parse(IRegistry.getIXmlParserFactory().newInputSource(in)), selectExpression);
	...
	public void moveFirst() throws JRException {
		...
		logger.fine("Selected nodes from " + document + " with expression: " + selectExpression + ", found: " + nodeListLength);
	}
	...
	public Object getFieldValue(JRField jrField) throws JRException 
	{
		...
		{
			logger.warning("Missing filed description for '" + jrField.getName() + "' in jrxml file, thus no data selected.");
			return null;
		}
		Object value = null;
		...
		if (logger.isLoggable(Level.FINEST)) {			
			logger.finest("Selected object from " + currentNode + " with expression: " + expression + ", found: " + selectedObject);
		}
