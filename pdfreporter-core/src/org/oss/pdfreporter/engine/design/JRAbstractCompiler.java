 ...
 * @version $Id: JRAbstractCompiler.java 5661 2012-09-13 19:00:14Z teodord $
 */
public abstract class JRAbstractCompiler implements JRCompiler
{
	...
	public final JasperReport compileReport(JasperDesign jasperDesign) throws JRException
	{
		...
		logger.fine("Requires: " + units.length + " compilation units (Datasets: " + (datasets.size() + 1) + ", Crosstabs: " + crosstabs.size() + ")");
		...
		logger.finest("Main Dataset generated: " + units[0].getName());
		...
		for (Iterator<JRDataset> it = datasets.iterator(); it.hasNext(); ++sourcesCount)
		{
			...
			logger.finest("Subdataset generated: " + units[sourcesCount].getName());
		}
		...
		for (Iterator<JRCrosstab> it = crosstabs.iterator(); it.hasNext(); ++sourcesCount)
		{
			...
			logger.finest("Crosstab dataset generated: " + units[sourcesCount].getName());
		}
