...
public class JRFillElementGroup implements JRElementGroup, JRFillCloneable
{
	...
	private final static Logger logger = Logger.getLogger(JRFillElementGroup.class.getName());
	/**
	 *
	 */
		...
		if (elementGrp != null)
		{
			...
			if (list != null && list.size() > 0)
			{
				for(int i = 0; i < list.size(); i++)
				{
					...
					if (child instanceof JRBaseTextField && ((JRBaseTextField)child).getExpression()!=null) {				
						logger.finest("addChild: " + child + ", Expression: " + ((JRBaseTextField)child).getExpression().getId());
					} else {
						logger.finest("JRElementGroup-addChild: " + child);
					}
