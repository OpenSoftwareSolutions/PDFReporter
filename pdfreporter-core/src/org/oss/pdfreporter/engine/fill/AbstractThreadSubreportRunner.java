 ...
 * @version $Id: AbstractThreadSubreportRunner.java 5747 2012-10-30 12:51:36Z lucianc $
 */
public abstract class AbstractThreadSubreportRunner extends JRSubreportRunnable implements JRSubreportRunner
{
	
	private static final Logger log = Logger.getLogger(AbstractThreadSubreportRunner.class.getName());
	...
	public JRSubreportRunResult resume()
	{
		...
		{
			log.finest("Fill " + subreportFiller.fillerId + ": notifying to continue");
		}
	...
	protected JRSubreportRunResult waitResult()
	{
		if (log.isLoggable(Level.FINEST))
		{
			log.finest("Fill " + subreportFiller.fillerId + ": waiting for fill result");
		}
		...
		catch (InterruptedException e)
		{
			if (subreportFiller.fillContext.isCanceled())
			{
				...
				if (log.isLoggable(Level.FINEST))
				{
					log.log(Level.FINEST,"Fill " + subreportFiller.fillerId + ": exception", e);
				}
			}
			else
			{
				log.log(Level.SEVERE,"Fill " + subreportFiller.fillerId + ": exception", e);
			}
		...
		if (log.isLoggable(Level.FINEST))
		{
			log.finest("Fill " + subreportFiller.fillerId + ": notified of fill result");
		}
	...
	public void cancel() throws JRException
	{
		if (log.isLoggable(Level.FINEST))
		{
			log.finest("Fill " + subreportFiller.fillerId + ": notifying to continue on cancel");
		}
		...
		if (isRunning())
		{
			if (log.isLoggable(Level.FINEST))
			{
				log.finest("Fill " + subreportFiller.fillerId + ": still running, waiting");
			}
			...
			if (log.isLoggable(Level.FINEST))
			{
				log.finest("Fill " + subreportFiller.fillerId + ": wait ended");
			}
		}
	}
	...
	public void suspend() throws JRException
	{
		if (log.isLoggable(Level.FINEST))
		{
			log.finest("Fill " + subreportFiller.fillerId + ": notifying on suspend");
		}
		...
		if (log.isLoggable(Level.FINEST))
		{
			log.finest("Fill " + subreportFiller.fillerId + ": waiting to continue");
		}
		...
		if (log.isLoggable(Level.FINEST))
		{
			log.finest("Fill " + subreportFiller.fillerId + ": notified to continue");
		}
	}
	...
	public void run()
	{
		...
		if (log.isLoggable(Level.FINEST))
		{
			log.finest("Fill " + subreportFiller.fillerId + ": notifying of completion");
		}
