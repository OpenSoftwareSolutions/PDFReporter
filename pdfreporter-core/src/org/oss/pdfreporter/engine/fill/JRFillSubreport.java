 ...
 * @version $Id: JRFillSubreport.java 5812 2012-11-19 18:29:25Z lucianc $
 */
public class JRFillSubreport extends JRFillElement implements JRSubreport
{
	...
	protected JasperReport evaluateReport(byte evaluation) throws JRException
	{
		...
		if (source != null) // FIXME put some default broken image like in browsers
		{
			...
			{
				if (source instanceof org.oss.pdfreporter.engine.JasperReport)
				{
				...
				{
					report = (JasperReport)JRLoader.loadObject((IURL)source);
				}
				...
				{
					report = SubreportUtil.loadSubreport((String)source);
...
//							);
	...
	{
		return (DatasetExpressionEvaluator) JasperCompileManager.getInstance(filler.getJasperReportsContext()).getEvaluator(jasperReport);
	}
